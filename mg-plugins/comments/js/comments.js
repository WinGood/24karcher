var comments = (function(){
	return {
		init: function(){
			...
			$('.add-cmt-form .sendComment').on('click', function(e){
    ...
    indicatorCount: function(count) {       
      ...
      $('.add-cmt-form .comments-msg').html('Подождите, идет отправка комментария...');
      $('.add-cmt-form .comments-msg').addClass('active');
      $.ajax({
			type: "POST",
			url: mgBaseDir+"/ajax",
			data: {
          ...
          name: $('.add-cmt-form input[name=name]').val(),
				email: $('.add-cmt-form input[name=email]').val(),
			  	comment: $('.add-cmt-form textarea').val(),
			  	captha: $('input[name="intCode"]').val()
			},
			dataType: "json",
			cache: false,
        ...
        if(response.status!="error")
        {
            ...
            $('.add-cmt-form .comments-msg').addClass('success')
        }
        else
        {
        	$('.add-cmt-form .comments-msg').addClass('error')
        }
        $('.add-cmt-form .comments-msg').html(response.msg);
        $('.add-cmt-form .comments-msg').addClass('active');
      }});
