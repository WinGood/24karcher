 ...
 *
 * @author HollowJ <HollowJ33@gmail.com>
 */
class Comments extends Actioner {
  ...
  public function addComment() {
    ...
    if (!empty($_POST['name'])&&!empty($_POST['email'])&&!empty($_POST['comment'])) {
      ...
      if(!preg_match('/^[A-Za-z0-9._-]+@[A-Za-z0-9_-]+.([A-Za-z0-9_-][A-Za-z0-9_]+)$/', $email)){     
    ...
    DB::query($sql);
    $this->messageSucces = '
      <p>Комментарий был успешно отправлен!</p>
      <span>Он будет опубликован после проверки модератором!</span>';
    return true;
  }
