...
class Filter {
  ...
  public function getHtmlFilter($submit = false) {
    ...
    $html .= '<div class="mg-filter-head filter-form">';
    $html .= '<div class="filter-preview"><div class="loader-search"></div><span></span></div>';
    foreach ($this->property as $name => $prop) {
      switch ($prop['type']) {
        case 'select': {
            if($name == 'sorter') continue;

            $html .= '<div class="wrapper-field"><div class="filter-select"><div class="select"><div class="label-field page-sidebar-title" style="border-bottom: 1px solid #f4f4f4;">'.$prop['label'].':</div><select name="'.$name.'" class="select-style last-items-dropdown">';
        ...
        case 'beetwen': {
           ...
           ';
              ...
              $html .= '
                <div class="page-sidebar-title">Цена</div>
                <div class="wrapper-field">
                <div class="price-slider-wrapper">
                <ul class="price-slider-list">
                 <li><input type="text" id="minCost" class="input-style price-input start-'.$prop['class'].'  price-input" data-fact-min="'.$prop['factMin'].'" name="'.$name.'[]" value="'.$prop['min'].'" /></li>
                 <li><input type="text" id="maxCost" class="input-style price-input end-'.$prop['class'].'  price-input" data-fact-max="'.$prop['factMax'].'" name="'.$name.'[]" value="'.$prop['max'].'" /></li>
                 <div class="clear"></div>
                 <div style="margin-top:15px; margin-left: 5px;margin-right:5px;margin-bottom:10px;"><div id="price-slider"></div></div>
                </ul>
              </div>
              </div>';
            }
  ...
  public function getHtmlPropertyFilter() { 
    ...
    foreach ($property as $idProp => $prop) {
      ...
      if($propCount>$maxCountProp){
        $style = "display:none";   
        $allFilter = '<div style="margin:8px 0;text-align: center"><a href="javascript:void(0);" class="mg-viewfilter-all">Показать все параметры</a></div>';
      }
          ...
          foreach ($values as $value) {          
            ...
            if(!empty($value)){
              ...
              if(!empty($this->accessValues[$idProp]) && in_array($value, $this->accessValues[$idProp])){     
                ...
                $html .= ' <li style="'.$style.'"><input  type="checkbox" id="prop-'.$idProp.'-'.$value.'" name="prop['.$idProp.'][]" value="'.$value.'" '.$checked.'  class="mg-filter-prop-checkbox"/><label for="prop-'.$idProp.'-'.$value.'"><span></span>'.$value.'</label>'.$viewAll.'</li>';
              }elseif(empty($this->accessValues[$idProp])){  
                $value = htmlspecialchars($value);
                $html .= ' <li style="'.$style.'"><input type="checkbox" id="prop-'.$idProp.'-'.$value.'" name="prop['.$idProp.'][]" value="'.$value.'" '.$checked.'  class="mg-filter-prop-checkbox"/><label for="prop-'.$idProp.'-'.$value.'"><span></span>'.$value.'</label>'.$viewAll.'</li>';
              }else{
                $value = htmlspecialchars($value);
                $html .= ' <li style="'.$style.'"><input disabled type="checkbox" name="prop['.$idProp.'][]" value="'.$value.'" '.$checked.'  class="mg-filter-prop-checkbox"/><label for=""><span></span>'.$value.'</label>'.$viewAll.'</li>';
               }
              $i++; 
            }
          }
