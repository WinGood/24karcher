...
class Models_Catalog {
  ...
  public function getMainCatProduct($catList)
  {
    $res = DB::query('
      SELECT
        DISTINCT p.id,
      ...
      ) AS temp ON p.id = temp.product_id WHERE (p.cat_id IN('.$catList.')) AND (p.count > 0 OR p.count < 0) ORDER BY id ASC LIMIT 4
    ');
  ...
  public function getMainCatInfo($catList)
  {
    $this->getCurrentCategory();

    $res = DB::query('
      SELECT id, title, url, parent_url
      FROM `'.PREFIX.'category`
      WHERE id IN('.$catList.')
    ');
    ...
    {
      $data[$i] = $category[$i];
      $data[$i]['count'] .= empty($countProduct[$i]['COUNT(*)']) ? 0 : $countProduct[$i]['COUNT(*)'];
    }
   ...
   * @return type
   */
  public function getList($countRows = 20, $mgadmin = false, $onlyActive = false, $sort, $isStock) {
  ...
  /**
  * Возвращает случайные продукты из данной категории
  */

  public function getRandomProduct($catId = null, $productId = null)
  {
    $res = DB::query("SELECT id FROM `".PREFIX."product` WHERE cat_id = $catId AND id != $productId");
    $products = array();
    if(DB::numRows($res) != 0)
    {
      
      while ($row = DB::fetchAssoc($res)) $products[$row['id']] = $row['id'];
      if(count($products) > 4) $products = array_rand($products, 4);
  ...
  public function getListProductByKeyWord($keyword, $allRows = false, $onlyActive = false, $adminPanel = false, $sort = NULL, $isStock = NULL, $count = NULL) {
    ...
    $navigator = new Navigator($sql, $page, $count, $linkCount = 6, $allRows); // Определяем класс.
    $this->pages = $navigator->getRowsSql();
    $idsProduct = array();
